...
Each scheduled run uses `--resume`. Version-7 checkpoints are migrated losslessly to version 8, questionable legacy candidates are rechecked, and removed noisy targets are archived. Resolved targets are skipped, never-searched rows are not delayed by stale backoff timestamps, and a fair category round-robin prevents starvation. Four bounded workers process targets concurrently, while a lock enforces the search budget and the workflow concurrency lock prevents overlapping runs.
