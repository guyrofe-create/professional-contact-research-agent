...
def valid_email(email):
    ...
    local,domain=email.rsplit("@",1); return bool(local and domain and "." in domain and local not in BAD_LOCAL and not domain.endswith((".png",".jpg",".jpeg",".webp")))
...
def search_queries(name,category):
    terms=CATEGORY_CONFIG.get(category,{}).get("terms",[category]); profession=" ".join(terms[:2]); queries=[f'"{name}" {profession}',f'"{name}" {profession} מייל',f'"{name}" {profession} צור קשר',f'"{name}" {profession} אתר רשמי',f'"{name}" email',f'"{name}" contact']
    if category in {"gynecologist","fertility_doctor"}: queries += [f'"{name}" site:{d}' for d in DIRECTORY_DOMAINS]
...
def extract(url,html):
    ...
    text=soup.get_text(" ",strip=True); found=[]
    ...
    title=soup.title.get_text(" ",strip=True) if soup.title else ""; return list(dict.fromkeys(found)),list(dict.fromkeys(links))[:5],text[:60000],title
def candidate_score(email,url,page_text,title,context,name,category,verified_site):
    kind=CATEGORY_CONFIG.get(category,{"kind":"person"}).get("kind","person"); local,email_domain=email.rsplit("@",1); page_identity=name_match(name,title+" "+page_text[:15000]); near_identity=name_match(name,context); profession=category_match(category,title+" "+page_text[:20000]); same_domain=registeredish(email_domain)==registeredish(host(url)); free_mail=email_domain in FREE_MAIL
    if not page_identity or not profession:return None
    if kind=="person":
        if local in GENERIC_LOCAL and not near_identity and not verified_site:return None
        ...
        score=55+(20 if near_identity else 0)+(10 if local_name_match(email,name) else 0)+(10 if same_domain else 0)+(5 if verified_site else 0)-(10 if local in GENERIC_LOCAL else 0)
    else:
        if not (same_domain or near_identity):return None
...
def research(row):
    name=str(row.get("name","")).strip(); category=str(row.get("category","")).strip(); seed_source=str(row.get("seed_source","")).strip(); config=CATEGORY_CONFIG.get(category,{"priority":"","kind":"person"}); attempts=[]; candidates=[]
    for hit in search_web(name,category,seed_source):
        url,html=fetch(hit["url"]); attempts.append(url)
        if not html:continue
        items,links,page_text,title=extract(url,html); verified_site=allowed_identity_page(url,title,page_text,name,category)
        ...
        for link in links:
            url2,html2=fetch(link); attempts.append(url2)
        ...
        if any(x[0]>=90 for x in candidates):break
    candidates=sorted(set(candidates),reverse=True); base={"algo_version":ALGO_VERSION,"name":name,"category":category,"priority":config.get("priority",""),"target_kind":config.get("kind",""),"seed_source":seed_source}
    if candidates:
        score,email,source,evidence,query,method=candidates[0]; return base|{"email":email,"email_type":classify(email,category),"confidence":score,"source_url":source,"status":"VERIFIED","evidence":evidence,"matched_query":query,"extraction_method":method,"attempted_urls":json.dumps(list(dict.fromkeys(attempts)),ensure_ascii=False)}
...
def excel_safe_frame(frame):
    ...
    return safe
def main():
    parser=argparse.ArgumentParser(); parser.add_argument("input"); parser.add_argument("--out",default="output"); parser.add_argument("--resume",action="store_true"); args=parser.parse_args(); out=Path(args.out); out.mkdir(parents=True,exist_ok=True); checkpoint=out/"checkpoint.jsonl"; done={}
    ...
    frame=frame.sort_values(["priority","status","confidence"],ascending=[True,True,False]).drop_duplicates(subset=["name","category"],keep="first"); person=frame[frame.target_kind=="person"]; repeated={email for email,count in Counter(person[person.email!=""].email).items() if count>2}
    if repeated:
    ...
    found=frame[frame.status=="VERIFIED"].copy().sort_values(["priority","confidence"],ascending=[True,False]).drop_duplicates(subset=["email"],keep="first"); found.to_csv(out/"contacts.csv",index=False,encoding="utf-8-sig"); excel_safe_frame(found).to_excel(out/"contacts.xlsx",index=False); excel_safe_frame(frame[frame.status.str.startswith("REVIEW")]).to_excel(out/"review.xlsx",index=False)
