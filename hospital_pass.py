...
# Official Israeli hospital / medical-center domains. Hospital pages are treated as
# first-party institutional sources, not general web directories.
HOSPITAL_DOMAINS = (
...
ELIGIBLE = {"gynecologist", "fertility_doctor", "fertility_nurse", "midwife", "embryologist"}
...
def hospital_research(row: dict):
    ...
    for query in queries(name, category):
        ...
        for hit in results:
            ...
            if not html:
                continue
            items, links, text, title = agent.extract(url, html)
            ...
            for email, context, method in items:
                score = agent.candidate_score(email, url, text, title, context, name, category, True)
            ...
            for link in links:
                ...
                if not h2:
                    continue
                items2, _, text2, title2 = agent.extract(u2, h2)
                for email, context, method in items2:
                    score = agent.candidate_score(email, u2, text2, title2, context, name, category, True)
                    if score is not None:
...
def main():
    ...
    for i, row in enumerate(rows):
        if row.get("category") not in ELIGIBLE or row.get("status") == "VERIFIED" or row.get("hospital_pass"):
            continue
